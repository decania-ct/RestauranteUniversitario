...
//
//  Created by Felipe Podolan Oliveira on 31/03/17.
...
/**
 Controller for the screen that searches for a ticket
 */
class TicketSearcherViewController: UIViewController, UIPickerViewDataSource, UIPickerViewDelegate {
    ...
    /// the UIView that holds the UIScrollView
    ...
    */
    func didBecomeActive() {
        ...
        let tap: UITapGestureRecognizer = UITapGestureRecognizer(target: self, action: #selector(TicketSearcherViewController.dismissKeyboard))
    ...
    func sendFunction() {
        if textID.text != "" {
            if pickerDataSource.count > 0 {
                ...
                //create the ticket object from the params
                let ticket = Ticket(user_id: user_id, queue: queue_id)
                //call the search ticket method from the API
                API.searchTicket(attendance: attendance, ticket: ticket,
                success: {tickets in
                    //if there is only one returned ticket, display the TicketView
                    if(tickets.count == 1) {
                        ...
                        ticketViewController.ticket = tickets[0] as! NSDictionary
                        ticketViewController.attendance = attendance
                        DispatchQueue.main.async {
                            self.navigationController?.pushViewController(ticketViewController, animated: true)
                        }
                    }
                    //if there is more than one returned tickets, display the SearchResultView
                    else {
                        ...
                        searchResultViewController.tickets = tickets
                        searchResultViewController.attendance = attendance
                        DispatchQueue.main.async {
                ...
                failure: { onError in
                    DispatchQueue.main.async {
                        let style = ToastStyle()
                        self.scrollView.makeToast(onError, duration: 5.0, position: .bottom, style: style)
                    }
                })
            }
            else {
